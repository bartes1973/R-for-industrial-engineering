### PROCESS MAP ###

# Step 1 - Import packages

library(grid)
library(SixSigma)

# Step 2 - Declare Inputs

inputs <- c("","","","","")

# Step 3 - Declare Outputs

outputs <- c("","","","","")

# Step 4 - Declare Steps

steps <- c("","","","","")

#Step 5 - Save the names of the inputs of each step in lists

io <- list()

io[[1]] <- list("","","")
io[[2]] <- list("","","")
io[[3]] <- list("","","")
io[[4]] <- list("","","")
io[[5]] <- list("","","")

#Step 6 - Save the names, parameter types and features

# LEGEND:

# C = Controllable
# Cr = Critical
# N = Noise
# P - Procedure

param <- list()
param[[1]] <- list(c("","C"),
                   c("","Cr"),
                   c("","N"),
                   c("","P"))
param[[2]] <- list(c("","C"),
                   c("","Cr"),
                   c("","N"),
                   c("","P"))
param[[3]] <- list(c("","C"),
                   c("","Cr"),
                   c("","N"),
                   c("","P"))
param[[4]] <- list(c("","C"),
                   c("","Cr"),
                   c("","N"),
                   c("","P"))
param[[5]] <- list(c("","C"),
                   c("","Cr"),
                   c("","N"),
                   c("","P"))

feat <- list()
feat[[1]] <- list("","","")
feat[[2]] <- list("","","")
feat[[3]] <- list("","","")
feat[[4]] <- list("","","")
feat[[5]] <- list("","","")

#Step 7 - Draw Process Map with the Six Sigma package

ss.pMap(steps,inputs,outputs,io,param,feat,sub = "")

# EXAMPLE

# Step 1 - Import packages

library(grid)
library(SixSigma)

# Step 2 - Declare Inputs

inputs <- c("Ingredients","Cook","Oven","Plates")

# Step 3 - Declare Outputs

outputs <- c("Temperature","Taste","Tenderness","Weight","Radius","Time")

# Step 4 - Declare Steps

steps <- c("DOUGH","TOPPINGS","BAKE","DELIVER")

#Step 5 - Save the names of the inputs of each step in lists

io <- list()

io[[1]] <- list("X's")
io[[2]] <- list("Dough","Ingredients","Cooker")
io[[3]] <- list("Raw Pizza","Cooker","Oven Plate")
io[[4]] <- list("Baked Pizza","Plate")

#Step 6 - Save the names, parameter types and features

param <- list()
param[[1]] <- list(c("Cook","C"),
                   c("Flour brand","C"),
                   c("Prop water","P"))
param[[2]] <- list(c("Cook","C"),
                   c("Ing. Brand","Cr"),
                   c("Amount","P"))
param[[3]] <- list(c("Cook","C"),
                   c("Queue","N"),
                   c("Baketime","Cr"))
param[[4]] <- list(c("Waiter","C"),
                   c("Queue","N"))

feat <- list()
feat[[1]] <- list("Density","Toughness","Thickness")
feat[[2]] <- list("Diameter","Weight","Thickness")
feat[[3]] <- list("Tmperature","Tenderness","Taste")
feat[[4]] <- list("Temperature","Taste","Tenderness","Weight","Radius","Time")

#Step 7 - Draw Process Map with the Six Sigma package

ss.pMap(steps,inputs,outputs,io,param,feat,sub = "Pizza Process Project")
