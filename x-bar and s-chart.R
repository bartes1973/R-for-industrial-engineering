...
# The x-bar and the s-chart are used for continuous data where the sample size (n) is > 10
...
sample_size <- as.integer(readline(prompt = "Type in the sample size n (must divide the total number of data points and be greater than 10): "))

# Validate the sample size (must divide the total number of data points and be greater than 10)
while((length(df[,1])%%sample_size != 0) | (sample_size < 11)){
  sample_size <- as.integer(readline(prompt = paste("Error! Type in an integer number greater than 10 that divides",length(df[,1]),": ")))
}
...
# Get the summary for the s-chart
summary(s_chart)
